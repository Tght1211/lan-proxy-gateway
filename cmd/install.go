...
func runInstall(cmd *cobra.Command, args []string) {
	...
	if allOK {
		...
		fmt.Println()
		fmt.Println("  默认工作台:  sudo gateway start")
		fmt.Println("  简单模式:    sudo gateway start --simple")
		fmt.Println()
		color.New(color.Bold).Println("  常用入口")
		fmt.Println()
		fmt.Println("  重新进入工作台:  sudo gateway console")
		fmt.Println("  重新进入简单模式: sudo gateway console --simple")
